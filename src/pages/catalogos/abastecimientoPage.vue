<template>
  ...
  >
    <q-card style="width: 100%">
      ...
      <q-item>
        <q-item-section>
          <abastecimiento-form ref="edit" :abastecimiento="selectedItem" />
        </q-item-section>
      </q-item>
    </q-card>
  </q-dialog>
...
const putItem = async () => {
  ...
  let res = await sendRequest(
    "PUT",
    final,
    "/api/intranet/abastecimiento/" + final.id,
    ""
  );
...
const destroyItem = async () => {
  let res = await sendRequest(
    "DELETE",
    null,
    "/api/intranet/abastecimiento/" + selectedItem.value.id,
    ""
  );
