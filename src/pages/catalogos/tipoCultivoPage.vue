<template>
  <BaseCatalogo
    ...
    :columns="columns"
    url="/api/intranet/tipoCultivos"
    :on-create="createItem"
    :on-update="updateItem"
    @delete="destroyItem"
  >
    <template #create-form>
      <TipoCultivoForm ref="add" />
    </template>

    <template #edit-form="{ item }">
      <TipoCultivoForm ref="edit" :tipoCultivo="item" />
    </template>
  </BaseCatalogo>
</template>
...
const BASE_URL = "/api/intranet/tipoCultivo"; // singular para post/put/delete

const createItem = async () => {
  const ok = await add.value.validate();
  if (!ok) return false;

  const data = { ...add.value.formTipoCultivo };
...
const updateItem = async (item) => {
  ...
  const data = { ...edit.value.formTipoCultivo, id: item.id };
