<template>
  ...
  >
    <q-card style="width: 100%">
      ...
      <q-item>
        <q-item-section>
          <cultivo-form ref="edit" :cultivo="selectedItem" />
        </q-item-section>
      </q-item>
    </q-card>
  </q-dialog>
...
const getRows = async () => {
  let res = await sendRequest("GET", null, "/api/intranet/cultivo", "");
  rows.value = res;
};
...
const postItem = async () => {
  ...
  let res = await sendRequest("POST", final, "/api/intranet/cultivo", "");
  showAdd.value = false;
  getRows();
};
...
const destroyItem = async () => {
  let res = await sendRequest(
    "DELETE",
    null,
    "/api/intranet/cultivo/" + selectedItem.value.id,
    ""
  );
