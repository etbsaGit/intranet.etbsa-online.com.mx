<template>
  ...
  >
    <q-card style="width: 100%">
      ...
      <q-item>
        <q-item-section>
          <riego-form ref="edit" :riego="selectedItem" />
        </q-item-section>
      </q-item>
    </q-card>
  </q-dialog>
...
import RiegoForm from "src/components/catalogos/RiegoForm.vue";
...
const postItem = async () => {
  ...
  let res = await sendRequest("POST", final, "/api/intranet/riego", "");
  showAdd.value = false;
  getRows();
};
...
const destroyItem = async () => {
  let res = await sendRequest(
    "DELETE",
    null,
    "/api/intranet/riego/" + selectedItem.value.id,
    ""
  );
