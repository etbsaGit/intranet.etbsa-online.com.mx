<template>
  ...
  >
    <q-card style="width: 100%">
      ...
      <q-item>
        <q-item-section>
          <kinship-form ref="edit" :kinship="selectedItem" />
        </q-item-section>
      </q-item>
    </q-card>
  </q-dialog>
...
const postItem = async () => {
  ...
  const final = {
    ...add.value.formKinship,
  };
  let res = await sendRequest("POST", final, "/api/intranet/kinship", "");
  showAdd.value = false;
  getRows();
};
...
const putItem = async () => {
  ...
  const final = {
    ...edit.value.formKinship,
  };
  let res = await sendRequest(
    "PUT",
    final,
    "/api/intranet/kinship/" + final.id,
    ""
  );
...
const destroyItem = async () => {
  let res = await sendRequest(
    "DELETE",
    null,
    "/api/intranet/kinship/" + selectedItem.value.id,
    ""
  );
