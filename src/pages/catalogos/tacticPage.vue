<template>
  ...
  >
    <q-card style="width: 100%">
      ...
      <q-item>
        <q-item-section>
          <tactic-form ref="edit" :tactic="selectedItem" />
        </q-item-section>
      </q-item>
    </q-card>
  </q-dialog>
...
const getRows = async () => {
  let res = await sendRequest("GET", null, "/api/intranet/tactic", "");
  rows.value = res;
};
...
const postItem = async () => {
  ...
  const final = {
    ...add.value.formTactic,
  };
  let res = await sendRequest("POST", final, "/api/intranet/tactic", "");
  showAdd.value = false;
  getRows();
};
...
const putItem = async () => {
  ...
  const final = {
    ...edit.value.formTactic,
  };
  let res = await sendRequest(
    "PUT",
    final,
    "/api/intranet/tactic/" + final.id,
    ""
  );
...
const destroyItem = async () => {
  let res = await sendRequest(
    "DELETE",
    null,
    "/api/intranet/tactic/" + selectedItem.value.id,
    ""
  );
