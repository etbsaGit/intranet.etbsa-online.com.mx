<template>
  <BaseCatalogo
    title="Segmentacion"
    :columns="columns"
    url="/api/intranet/segmentations"
    :on-create="createItem"
    :on-update="updateItem"
    @delete="destroyItem"
  >
    <template #create-form>
      <SegmentationForm ref="add" />
    </template>

    <template #edit-form="{ item }">
      <SegmentationForm ref="edit" :segmentation="item" />
    </template>
  </BaseCatalogo>
</template>
...
const BASE_URL = "/api/intranet/segmentation"; // singular para post/put/delete

const createItem = async () => {
  const ok = await add.value.validate();
  if (!ok) return false;

  const data = { ...add.value.formSegmentation };
...
const updateItem = async (item) => {
  ...
  const data = { ...edit.value.formSegmentation, id: item.id };
