<template>
  <BaseCatalogo
    title="Condiciones"
    :columns="columns"
    url="/api/intranet/condiciones"
    :on-create="createItem"
    :on-update="updateItem"
    @delete="destroyItem"
  >
    <template #create-form>
      <CondicionForm ref="add" />
    </template>

    <template #edit-form="{ item }">
      <CondicionForm ref="edit" :condicion="item" />
    </template>
  </BaseCatalogo>
</template>
...
const BASE_URL = "/api/intranet/condicion"; // singular para post/put/delete
